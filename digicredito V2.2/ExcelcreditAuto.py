...
def idSimulacion(idCredito):
    try:
        ...
        idSim =cursor.fetchall()
    finally:
        cursor.close()
        conexcion.close()
        return idSim[0][0]

def obtenerEstadoCredito(idCredito):
    try:
        conexcion = psycopg2.connect(user=cr.user, password=cr.password, 
                                    host=cr.host, 
                                    port=cr.port, database=cr.database) 
        cursor = conexcion.cursor()
        cursor.execute(f"select estado from credito where id ={idCredito};")
...
def notifyCompletion(idCredito,head):
    try:
        i = 3
        estado = obtenerEstadoCredito(idCredito=idCredito)
        while i > 0 and estado == 'PROSPECCION_EXITOSA':
            i = i -1
            ...
            if response.status_code != 200:
                raise Exception
            if i == 0 and estado == 'PROSPECCION_EXITOSA':
                cambiarEstado(idCredito=idCredito)
            logging.info('***PROSPECCION COMPLETADA***')
            return response.text
    except Exception as err:
        logging.info(err)
...
def habeasIdentidad(datos):
    ...
    sleep(30)
    notifyCompletion(idCredito=idCredito,head=head)
    sleep(30)
...
def datos_laborales_o_pensionales(driver, datos):
    try:
        ...
        driver.find_element(By.XPATH, '/html/body/div/main/div[2]/div[2]/div/div[2]/div[2]/div[2]/form/div[3]/div[2]/input').send_keys("Selenium Prueba")
        sleep(2)
...
def fima_final(driver):
    ...
    # Boton siguiente 
    driver.find_element(By.XPATH, '/html/body/div/main/div[2]/div[2]/div/div[2]/div[2]/div[2]/div[3]/button').click()
    sleep(5)


    # modulo 3
    driver.find_element(By.XPATH, '/html/body/div/main/div[2]/div[2]/div/div[2]/div[2]/div[2]/form/div[1]/div[1]/div').click()
    driver.find_element(By.ID, 'option0').click()
    sleep(5)
